...
class ReplyAppStateRestorationTest {
    ...
    fun compactDevice_selectedEmailEmailRetained_afterConfigChange() {
        ...
        // ... verify again that the third email is displayed in the details screen
        composeTestRule.onNodeWithText(
            composeTestRule.activity.getString(LocalEmailsDataProvider.allEmails[2].body)
        ).assertExists()
...
//        // Given third email is displayed
        composeTestRule.onNodeWithText(
           composeTestRule.activity.getString(LocalEmailsDataProvider.allEmails[0].body)
       ).assertIsDisplayed()

        // Select third email
        composeTestRule.onNodeWithText(
            composeTestRule.activity.getString(LocalEmailsDataProvider.allEmails[0].subject)
        ).performClick()

        // Verify that the details screen displays the third email by using the testTag on
